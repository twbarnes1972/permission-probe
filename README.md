...
```jsonc
{
  ...
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "python /absolute/path/to/app-src/file-deny-guard.py"
      }]
    }]
  }
}
...
Customize the `DENY_PATTERNS` list at the top of `app-src/file-deny-guard.py` for paths you want blocked. Windows / macOS / Linux examples are included as commented-out entries. Restart Claude Code once to register the hook; after that, pattern edits are live (no restart).

**Windows:** use forward slashes in the hook command path — `python C:/path/to/app-src/file-deny-guard.py`, not backslashes. The hook runner's shell parsing strips `\\` and you get fail-closed errors on every Read/Edit. Python on Windows accepts forward slashes.
