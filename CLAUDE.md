...
`permission-probe` is a **workaround tool** for a Claude Code bug: path-globbed `Edit(...)`, `Read(...)`, `Write(...)` rules in `~/.claude/settings.json` are silently ignored by the permission matcher. Only bare `Edit` / `Read` / `Write` rules work. Two artifacts: `file-deny-guard.py` (a `PreToolUse` hook that re-implements path-based denies) and `probe.js` (a diagnostic that proves the bug is not in picomatch). Full root-cause analysis is in the README and the upstream comment drafts under `docs/upstream-comments/`. This tool becomes obsolete when upstream lands a matcher fix.

## Maintenance guidance — when working on this repo

### The hook contract — don't break it

`file-deny-guard.py` is a Claude Code `PreToolUse` hook. Contract:
...
```bash
python -c "import json,sys; sys.stdout.write(json.dumps({'tool_name':'Edit','tool_input':{'file_path':'/etc/passwd'}}))" | python file-deny-guard.py
```
...
1. Verify with `probe.js` against the new claude.exe (the disassembly methodology in `docs/upstream-comments/upstream-comment-36884.md` still applies).
